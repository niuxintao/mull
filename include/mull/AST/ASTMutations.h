...
public:
  ...
  bool locationExists(SourceFilePath sourceFile, mull::MutatorKind mutatorKind,
                      int line, int column) const;

  const ASTMutation &getMutation(const std::string &sourceFile,
                                 mull::MutatorKind mutatorKind, int line,
                                 int column) const;

  void dump() const;

  void saveExpr(std::string &sourceFile,
