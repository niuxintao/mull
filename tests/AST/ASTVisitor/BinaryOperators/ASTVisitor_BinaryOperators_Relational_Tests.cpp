...
TEST(ASTVisitorTest, basicConditional) {
...
int add(int a, int b) {
  ...
  mull::TraverseMask traverseMask({mull::MutatorKind::CXX_Relation_LessThanToGreaterOrEqual});
  ...
  ASSERT_TRUE(astMutations.locationExists(fakeSourceFilePath, mull::MutatorKind::CXX_Relation_LessThanToGreaterOrEqual, 3, 12));
}
