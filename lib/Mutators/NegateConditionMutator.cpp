...
NegateConditionMutator::NegateConditionMutator() : lowLevelMutators(), negateXorMutator(), negateTruncMutator() {
  /// == -> !=
...
NegateConditionMutator::getMutations(Bitcode *bitcode, const FunctionUnderTest &function) {
  ...
  for (llvm::Instruction *instruction : function.getSelectedInstructions()) {
    ...
    if (negateTruncMutator.canMutate(instruction)) {
      std::string diagnostics = "Negate Condition: replaced x with !x";
      auto point =
        new MutationPoint(this, &negateTruncMutator, instruction, "", bitcode, diagnostics);
      mutations.push_back(point);
    }
